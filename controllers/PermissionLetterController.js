...
const getParentPermissionLetters = async (req, res) => {
  try {
    const permissionLetters = await PermissionLetter.find({parent: req.user._id}).sort({createdAt: -1});
...
const getPermissionLetterById = async (req, res) => {
  try {
    ...
    if (req.user.role === "admin") {
      permissionLetter = await PermissionLetter.findById(id).populate("parent", "name email");
    } else {
...
const updatePermissionLetterStatus = async (req, res) => {
  try {
    ...
    const permissionLetter = await PermissionLetter.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true
    }).populate("parent", "name email");
...
const updatePermissionLetter = async (req, res) => {
  try {
    ...
    if (supportingDocument) updateFields.supportingDocument = supportingDocument;

    const permissionLetter = await PermissionLetter.findByIdAndUpdate(id, updateFields, {
      new: true,
      runValidators: true
    });
...
const deletePermissionLetter = async (req, res) => {
  try {
    ...
    const filter = req.user.role === "admin" ? {_id: id} : {_id: id, parent: req.user._id};
